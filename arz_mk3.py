...
class gui():
    ...
    def startexp(self):  # just for showing purposes manual inputs
        ...
        time.insert(curExNum, end-start)
        gravity.insert(curExNum, (height/time[curExNum]**2)*2) #calculating gravity
        pressure.insert(curExNum, self.adjustPressure(onLoop=True))
        exnum.insert(curExNum, 'Exp {0:d}'.format(curExNum+1))

        self.labelgravity.config(text="Gravity: {0:0.2f}".format(gravity[curExNum]))
        self.labeltime.config(text="Time: {0:0.2f}".format(time[curExNum]))
        self.labelpressure.config(text="Pressure: {0:0.2f}".format(pressure[curExNum]))
    ...
    def ptG(self):
        ...
        for i in range(0, int(time[curExNum-1] * sensitivity)):  # this is not correct
            currentTime.insert(i, math.sqrt(i * time[curExNum-1] / sensitivity))
            currentHeight.insert(i, (0.5 * gravity[curExNum-1] * currentTime[i] ** 2))
    ...
    def stG(self):
        ...
        for i in range(0, int(time[curExNum-1] * sensitivity)):
            currentTime.insert(i, math.sqrt(i * time[curExNum-1] / sensitivity))
            currentSpeed.insert(i, gravity[curExNum-1] * currentTime[i])
    ...
    def atG(self):
        ...
        for i in range(0, int(time[curExNum-1] * sensitivity)):
            currentTime.insert(i, math.sqrt(i * time[curExNum-1] / sensitivity))
            currentGravity.insert(i, gravity[curExNum-1])
        plt.figure('Arz Cazibesi')
